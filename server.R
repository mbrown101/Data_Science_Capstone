...
# newdata <- mtcars[order(mpg, cyl),]
  ...
  #ifelse(test, yes, no)
  #stri_stats_latex(input$stringInput)[4]
  ...
  termTail1 <- function() return( ifelse( !is.null(input$stringInput)  , word( input$stringInput , -1) , NULL ) )
...
#      arrange(oracle3gram[ which(oracle3gram$leadingGram == input$stringInput ) , ], -count)[1:input$sliderGuesses,]
#    }, options = list(orderClasses = TRUE , searching = FALSE , paging = FALSE))

    output$matchTable = renderDataTable({ 
      if (length(unlist(strsplit(input$stringInput , " "))) == 2){
        arrange(oracle3gram[ which(oracle3gram$leadingGram == tolower(trimws(input$stringInput)) ) , ], -count)[1:input$sliderGuesses,]
        }
      
      if (length(unlist(strsplit(input$stringInput , " "))) == 1){
        arrange(oracle2gram[ which(oracle2gram$leadingGram == tolower(trimws(input$stringInput)) ) , ], -count)[1:input$sliderGuesses,]
      }}, options = list(orderClasses = TRUE , searching = FALSE , paging = FALSE))    
